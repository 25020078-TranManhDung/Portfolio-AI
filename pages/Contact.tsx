...
export default function Contact() {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col gap-12 max-w-6xl mx-auto w-full pb-20"
    >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 mt-8 lg:mt-0">
            ...
            <div className="flex items-center gap-4 mt-2">
              <button 
                ...
                className={`bg-primary hover:bg-primary/90 text-primary-foreground font-semibold py-3 px-8 rounded-xl transition-all shadow-lg hover:shadow-primary/25 active:scale-[0.98] w-fit disabled:opacity-70 disabled:cursor-not-allowed ${isSuccess ? 'bg-green-500 hover:bg-green-600' : ''}`}
              >
                {isSubmitting ? "Đang Gửi..." : isSuccess ? "Đã Gửi Thành Công! ✓" : "Gửi Tin Nhắn"}
              </button>
            </div>
          </form>
        </div>
